<template>
  <div>
    ...
    <div v-transfer-dom>
      <aui-popup v-model="showPopup" height="100%" @hide="closePopup" class='popup-dialog'>
        <div class="popup">
          ...
          <aui-group>
            <dept-selection ref="dept" :column='{label:"部门筛选"}' placeholder='按部门搜索' @deptSelect="deptSelect" :disabled='loading' :value="queryDept" :hidden="hidden" @close="closeDept"></dept-selection>
          </aui-group>
            <aui-group title="用户列表">
              <aui-load-more v-if='loading' tip='数据加载中...'></aui-load-more>
              <aui-scroller ref="scroller" lock-x scrollbar-y v-model="scrllerStatus" use-pulldown use-pullup :pulldown-config="pulldownConfig" :pullup-config="pullupConfig" @pulldown-loading="reFresh" @pullup-loading="nextPage">
                ...
                <aui-load-more v-else-if="!userList.length && !loading" :show-loading="false" tip="暂无数据" background-color="#fbf9fe"></aui-load-more>
              </aui-scroller>
            </aui-group>
        </div>
        <!-- <div v-transfer-dom>
          <aui-loading :show="loading" text="加载中..."></aui-loading>
        </div> -->
      </aui-popup>
    </div>
