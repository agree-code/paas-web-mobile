<template>
  <div class="repay-issue">
    ...
    <p class="reiMoneyInfo">借款总额：¥1，000.00；已还款：¥0.00</p>
    <aui-group>
      <aui-input title="还款金额"
                 placeholder="输入金额"
                 text-align="right">
      </aui-input>
      <aui-popup-picker v-model="selValue"
                        title="还款类型"
                        :data="[['现金','转账','工资','报销冲账','固定资产冲账','项目内采冲账']]"
                        placeholder="请选择">
      </aui-popup-picker>
    </aui-group>
    <aui-group>
    ...
    <aui-group>
      <aui-textarea title="备注"
                    placeholder="请输入备注"></aui-textarea>
    </aui-group>
    <div class="foot-btn">
...
export default {
  data() {
    return {
      ...
      selValue:[],
      labelFirst:[{"name":"预算分类","value":[],"labelList":[['应用项目预算','研发项目预算','其他项目预算']]},{"name":"预算类型","value":[],"labelList":[['部门年度费用预算','其他项目预算']]},{"name":"预算名称","value":[],"labelList":[['xx项目']]},{"name":"还款形式","value":[],"labelList":[['对私转账','领取现金','支票']]}],
    };
  },
  mounted() {
    this.$store.state.headTitle = '还款申请';
...
.repay-issue .aui-cells {
  margin-top: 15px;
}
.repay-issue .aui-cells:before {
  border-top: 0;
}
.repay-issue .aui-cells:after {
  border-bottom: 0;
}
.repay-issue .aui-btn:after {
  border: none;
}
.repay-issue .aui-btn + .aui-btn {
  margin-top: 0;
}
.repay-issue .aui-btn:after {
  border: none;
